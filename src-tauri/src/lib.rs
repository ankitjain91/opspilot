...
struct ResourceCost {
    ...
    cpu_cores: f64,        // in vCPU cores
    memory_gb: f64,        // in GB
    cpu_cost_monthly: f64, // USD
    memory_cost_monthly: f64,
    total_cost_monthly: f64,
    pod_count: u32,        // for deployments
}

#[derive(Serialize, Clone)]
struct NamespaceCost {
    namespace: String,
    total_cost_monthly: f64,
    cpu_cost_monthly: f64,
    memory_cost_monthly: f64,
    cpu_cores: f64,
    memory_gb: f64,
    pod_count: u32,
    top_resources: Vec<ResourceCost>,
}
...
#[tauri::command]
async fn get_cluster_cost_report(state: State<'_, AppState>) -> Result<ClusterCostReport, String> {
    let client = create_client(state.clone()).await?;
    let pods_api: Api<k8s_openapi::api::core::v1::Pod> = Api::all(client.clone());
    ...
    for pod in pods.items {
        ...
        let entry = namespace_costs.entry(namespace).or_insert((0.0, 0.0, 0, Vec::new()));
        entry.0 += cpu_cores;
        entry.1 += memory_gb;
        entry.2 += 1;
    ...
    // Build namespace cost summaries
    let mut namespaces: Vec<NamespaceCost> = namespace_costs.into_iter().map(|(ns, (cpu, mem, count, resources))| {
        let cpu_cost = cpu * AZURE_CPU_PRICE_PER_CORE_HOUR * HOURS_PER_MONTH;
        let memory_cost = mem * AZURE_MEMORY_PRICE_PER_GB_HOUR * HOURS_PER_MONTH;

        // Aggregate resources by owner name
        let mut aggregated: std::collections::HashMap<String, ResourceCost> = std::collections::HashMap::new();
        for r in resources {
            let entry = aggregated.entry(r.name.clone()).or_insert(ResourceCost {
                name: r.name.clone(),
        ...
        let mut top_resources: Vec<ResourceCost> = aggregated.into_values().collect();
        top_resources.sort_by(|a, b| b.total_cost_monthly.partial_cmp(&a.total_cost_monthly).unwrap());
        top_resources.truncate(10); // Top 10 resources per namespace

        NamespaceCost {
            namespace: ns,
            total_cost_monthly: cpu_cost + memory_cost,
            cpu_cost_monthly: cpu_cost,
            memory_cost_monthly: memory_cost,
            cpu_cores: cpu,
            memory_gb: mem,
    ...
    namespaces.sort_by(|a, b| b.total_cost_monthly.partial_cmp(&a.total_cost_monthly).unwrap());

    // Calculate totals
    let total_cpu: f64 = namespaces.iter().map(|n| n.cpu_cores).sum();
    let total_memory: f64 = namespaces.iter().map(|n| n.memory_gb).sum();
    let total_pods: u32 = namespaces.iter().map(|n| n.pod_count).sum();
    let total_cpu_cost: f64 = namespaces.iter().map(|n| n.cpu_cost_monthly).sum();
    let total_memory_cost: f64 = namespaces.iter().map(|n| n.memory_cost_monthly).sum();

    Ok(ClusterCostReport {
        total_cost_monthly: total_cpu_cost + total_memory_cost,
        cpu_cost_monthly: total_cpu_cost,
        memory_cost_monthly: total_memory_cost,
