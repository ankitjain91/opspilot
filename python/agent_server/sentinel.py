...
class SentinelLoop:
    ...
    async def start(self):
        ...
        """
        self.running = True
        logger.info(f"[SHIELD] Sentinel starting for context: {self.kube_context or 'default'}")
        ...
        while self.running:
            ...
            try:
                ...
                logger.info(f"[SHIELD] Watching for Warning events on {self.kube_context or 'default'}...")
                ...
                logger.info(f"[SHIELD] Sentinel cancelled (context switch or shutdown)")
                self.running = False
                ...
                if should_log:
                    ...
                    if "Connection reset" in error_str or "Cannot connect" in error_str:
                        if self._consecutive_failures > 1:
                            logger.warning(f"[SHIELD] Sentinel: Cluster unreachable (attempt #{self._consecutive_failures}). Will keep retrying silently...")
                        else:
                            logger.warning(f"[SHIELD] Sentinel: Cluster appears unreachable - may require port-forward or VPN. Retrying in background...")
                        ...
                        logger.warning(f"[SHIELD] Sentinel: Local/vcluster connection failed ({error_type}). Ensure vcluster connect is running. Retrying in {backoff_time}s...")
                    elif "SSL" in error_str or "certificate" in error_str.lower():
                        logger.warning(f"[SHIELD] Sentinel: TLS/SSL error - {error_str[:100]}. Retrying in {backoff_time}s...")
                    else:
    ...
    async def stop(self):
        """Stop the loop."""
        self.running = False
        logger.info("[SHIELD] Sentinel stopped.")
    ...
    async def process_warning(self, event_obj):
        ...
        logger.info(f"[ALERT] SENTINEL ALERT: {alert_msg}")
